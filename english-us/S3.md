...
Amazon S3 (Simple Storage Service) is an object storage service that allows you to store and retrieve data from anywhere on the web. It is designed to scale infinitely, enabling you to store and retrieve massive amounts of data with high availability and durability.
...
In a simple analogy, you can compare a file in a folder on a hard drive to an object in a bucket on S3:
...
<ul>
    <li><b>Use Cases:</b> Used for Data lakes, data archiving, hosting static websites, etc.</li>
...
<ul>
<li><b>Buckets:</b> Containers for S3 objects. All objects are stored in a bucket. Before uploading objects, it is necessary to create a bucket.</li>
<li><b>Objects:</b> The data stored in S3 is called objects. Each object consists of data and metadata (information about the object).</li>
...
<li><b>Storage Classes:</b> S3 offers various storage classes for objects, each with different durability, availability, and cost characteristics. Some questions that can be asked: How often will the data be retrieved? Does the data need to be highly or less available? Depending on the answer, a class can be selected:
    ...
    <tr>
      ...
      <td>
        Ideal for long-term data, rarely accessed but requiring quick retrieval (milliseconds). Offers access as fast as Standard and Standard-IA. Ideal for data accessed once per quarter.
      </td>
    </tr>
    <tr>
      <td>S3 Glacier Flexible Retrieval</td>
      <td>
        For data that does not require immediate access. Ideal for non-urgent backup usage, disaster recovery. The user can choose the retrieval speed. Ideal for data accessed 1 or 2 times per year.
      </td>
    </tr>
    <tr>
      <td>S3 Glacier Deep Archive</td>
      <td>
        Supports long-term retention and digital preservation for data that may be accessed 1 or 2 times per year. Ideal for companies needing to retain data for legal compliance for 7 to 10 years. Data retrieval within 12 hours.
      </td>
    </tr>
  </table>
</li>
